...
离开座位前打开看板，通过大色块查看本机 Codex 会话：红色正在运行，绿色本轮正常结束。先验证实用性，后续再美化。
...
| --- | --- |
| 红色 | 本轮正在运行 |
| 绿色 | 收到正常结束事件，不代表整个项目验收通过 |
| 黄色 | 明确输入等待、中断或失败 |
| 灰色 | 读取失败，或运行中的记录超过 5 分钟没有更新 |

初始显示正在运行的会话，随后自动加入新开始的会话。结束后保留色块，再次运行自动变红。历史空闲会话、已归档会话和内部子代理不显示。列表随后台进程保留；停止再启动重新收录。
...
- 电脑睡眠时不会刷新；长时间推理但没有日志时会灰色待确认，后续记录恢复后自动更新。
