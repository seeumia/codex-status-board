...
离开座位前打开看板，通过大色块查看本机 Codex 会话：红色正在开发，绿色本轮完成待阅读。普通未读任务在 Codex 打开后移除；完成时已被自动标为已读的任务先保留，避免漏读。
...
| --- | --- |
| 红色 | 本轮正在运行 |
| 绿色 | 收到正常结束事件，且 Codex 标为未读；不代表整个项目验收通过 |

启动时收录状态可确认的运行中会话，以及已完成且仍未读的会话，不限制完成时间。普通未读任务在 Codex 打开、被标为已读后自动移除。看板运行期间观察到任务刚完成、但 Codex 已将其标为已读时，先保留绿色色块，直到该任务进入下一轮开发或其他明确生命周期状态；不把自动已读当成用户确实阅读。再次开发时变红。等待输入、中断、失败、超过 5 分钟无更新或读取异常的会话不显示色块，自动化任务（无论开发中、未读或已读）、已归档会话和内部子代理不显示。

**尚未实现：** 已被自动标为已读的任务“离开再进入后移除”。现有只读文件没有可靠的任务重新进入记录，因此暂时保留，页面注明“暂时保留，避免漏读”。此保护状态仅随后台进程保留，停止再启动会重新依据 Codex 未读列表收录。不能把这项保护宣称为完整的阅读识别。

## 实现与限制

- macOS / Windows，Python 3.9+；使用标准库，无第三方运行依赖。
- 会话生命周期读取原验证版本：Codex CLI 0.153.4；已读状态接入依据本机 0.153.3 随附桌面应用代码及实际持久化数据。只读本地 SQLite 元数据、JSONL 生命周期事件，以及 `.codex-global-state.json` 中 `electron-persisted-atom-state.unread-thread-ids-by-host-v1.local` 的未读任务 ID，不改 Codex 配置和会话，不向页面提供其他配置或对话正文。
...
- 内部存储格式可能随 Codex 升级变化，无法读取会话列表或已读状态时显示页顶提示，暂不显示任务色块；恢复后自动更新。没有把 idle 直接当完成。未读信息由桌面应用落盘后在下一次轮询同步；Windows 上的新已读联动尚需真实桌面验证。
- 自然语言追问、未写入日志的审批等待无法可靠识别；绿色仅指本轮正常结束且待阅读。
- 电脑睡眠时不会刷新；长时间推理但没有日志的任务会暂时隐藏，后续记录恢复后自动出现。
...
测试使用临时数据库和日志，不修改真实 Codex 会话。覆盖动态状态、启停、重复和并发启动、中文/空格路径及安装目录迁移。
